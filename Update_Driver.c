...
#define SPI_ADDRESS  "/dev/spislave0"
...
/**************GPIO*******************/
#define GPIO_0_ADDRESS "/dev/stdb_controller"
...
int spi_read_first_frame(int fd, uint16_t r_len, uint8_t *data){
	...
	if (len < 0){
		...
		if(ack_ret || 0x55 != data[0] || 0xAA != data[1]){
			...
			frame_print("\nrecieve first frame:",data,first_frame_len);	
			ret = 0;
		}
	}
	...
	while(recieve_flag){
		...
		if(ack_ret){
			...
			if(data[0] == 0x55 && data[1] == 0xAA && data[3] == 0x0b){
				Right_Gpio_Ack();
				time_debug("");
				frame_print("log:read first ack:", data, 10);
				break;	
			}else{
				continue;
			}
